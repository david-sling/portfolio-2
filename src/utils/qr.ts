...
class BitBuffer {
  ...
  append(value: number, bitCount: number) {
    for (let i = 0; i < bitCount; i++) {
      ...
      this.buffer[this.bitOffset >> 3] =
        (writeByte & ~writeMask) | (value & readMask ? writeMask : 0);
      this.bitOffset++;
    }
  }
  ...
  read(bitPosition: number) {
    const value =
      this.buffer[bitPosition >> 3] & (1 << (7 - (bitPosition & 7))) ? 1 : 0;
    return value;
  }
}
...
class SegmentNumeric {
  ...
  static canEncode(text: any) {
    return [...text].every((c) => SegmentNumeric.CHARSET.includes(c));
  }

  static payloadSize(text: string | any[]) {
    const charCount = text.length;
    return (
      10 * Math.floor(charCount / 3) +
      (charCount % 3) * 4 -
      Math.floor((charCount % 3) / 2)
    );
  }
  ...
  static totalSize(version: any, text: any) {
    return (
      Segment.MODE_BITS +
      SegmentNumeric.countSize(version) +
      SegmentNumeric.payloadSize(text)
    );
  }

  static encode(
    bitBuffer: { append: (arg0: number, arg1: number) => void },
    version: any,
    text: any
  ) {
...
class SegmentAlphanumeric {
  ...
  static totalSize(version: any, text: any) {
    return (
      Segment.MODE_BITS +
      SegmentAlphanumeric.countSize(version) +
      SegmentAlphanumeric.payloadSize(text)
    );
  }
...
class SegmentEightBit {
  ...
  static totalSize(version: any, text: any) {
    return (
      Segment.MODE_BITS +
      SegmentEightBit.countSize(version) +
      SegmentEightBit.payloadSize(text)
    );
  }
...
class Segment {
  ...
  text: any;
  mode:
    | typeof SegmentNumeric
    | typeof SegmentAlphanumeric
    | typeof SegmentEightBit;
...
class Matrix {
  ...
  static calculateMask(maskPattern: any, j: number, i: number) {
    switch (maskPattern) {
      case 0:
  ...
  static alignmentCoordinates(version: number) {
    ...
    const step =
      version == 32
        ? 26
        : Math.floor((version * 4 + count * 2 + 1) / (count * 2 - 2)) * 2; // step to previous
  ...
  drawFinder(ox: number, oy: number) {
    for (
      let y = -Math.floor(Matrix.FINDER_SIZE / 2) - 1;
      y <= Math.floor(Matrix.FINDER_SIZE / 2) + 1;
      y++
    ) {
      for (
        let x = -Math.floor(Matrix.FINDER_SIZE / 2) - 1;
        x <= Math.floor(Matrix.FINDER_SIZE / 2) + 1;
        x++
      ) {
        let value =
          (Math.abs(x) > Math.abs(y) ? Math.abs(x) : Math.abs(y)) & 1
            ? Matrix.MODULE_DARK
            : Matrix.MODULE_LIGHT;
  ...
  drawTiming() {
    const id = "Ti";
    for (
      let i = Matrix.FINDER_SIZE + 1;
      i < this.dimension - Matrix.FINDER_SIZE - 1;
      i++
    ) {
  ...
  drawAlignment(ox: number, oy: number) {
    for (let y = -Matrix.ALIGNMENT_RADIUS; y <= Matrix.ALIGNMENT_RADIUS; y++) {
      for (
        let x = -Matrix.ALIGNMENT_RADIUS;
        x <= Matrix.ALIGNMENT_RADIUS;
        x++
      ) {
        let value =
          1 - ((Math.abs(x) > Math.abs(y) ? Math.abs(x) : Math.abs(y)) & 1)
            ? Matrix.MODULE_DARK
            : Matrix.MODULE_LIGHT;
  ...
  populateFunctionPatterns() {
    this.drawFinder(
      Math.floor(Matrix.FINDER_SIZE / 2),
      Math.floor(Matrix.FINDER_SIZE / 2)
    );
    this.drawFinder(
      this.dimension - 1 - Math.floor(Matrix.FINDER_SIZE / 2),
      Math.floor(Matrix.FINDER_SIZE / 2)
    );
    this.drawFinder(
      Math.floor(Matrix.FINDER_SIZE / 2),
    ...
    for (let h of alignmentCoords) {
      for (let v of alignmentCoords) {
        ...
        if (
          h >= this.dimension - 1 - Matrix.FINDER_SIZE &&
          v <= Matrix.FINDER_SIZE
        )
          continue; // Obscured by top-right finder
        if (
          h <= Matrix.FINDER_SIZE &&
          v >= this.dimension - 1 - Matrix.FINDER_SIZE
        )
          continue; // Obscured by bottom-left finder
  ...
  cursorAdvance() {
    while (this.cursorX >= 0) {
      ...
      if ((this.cursorX & 1) ^ (this.cursorX > Matrix.TIMING_OFFSET ? 1 : 0)) {
        ...
        if (
          ((this.cursorX - (this.cursorX > Matrix.TIMING_OFFSET ? 1 : 0)) / 2) &
          1
        ) {
  ...
  drawFormatInfo(value: number) {
    ...
    for (let i = 0; i < 15; i++) {
      ...
      if (i < 6) this.setModule(Matrix.FINDER_SIZE + 1, i, v, id);
        ...
        this.setModule(Matrix.FINDER_SIZE + 1, Matrix.FINDER_SIZE, v, id);
      else if (i == 7)
        this.setModule(Matrix.FINDER_SIZE + 1, Matrix.FINDER_SIZE + 1, v, id);
      else if (i == 8)
        this.setModule(Matrix.FINDER_SIZE, Matrix.FINDER_SIZE + 1, v, id);
      else this.setModule(14 - i, Matrix.FINDER_SIZE + 1, v, id);

      // lower 8-bits starting LSB right-to-left underneath top-right finder
      if (i < 8)
        this.setModule(this.dimension - 1 - i, Matrix.FINDER_SIZE + 1, v, id);
      // upper 7-bits starting LSB top-to-bottom right of bottom-left finder
      else
        this.setModule(
          Matrix.FINDER_SIZE + 1,
          this.dimension - Matrix.FINDER_SIZE - 8 + i,
          v,
          id
        );
    }
    ...
    this.setModule(
      Matrix.FINDER_SIZE + 1,
      this.dimension - 1 - Matrix.FINDER_SIZE,
      Matrix.MODULE_DARK,
      id
    );
  }
  ...
  drawVersionInfo(value: number | null) {
    ...
    for (let i = 0; i < 18; i++) {
      ...
      this.setModule(
        col,
        this.dimension - 1 - Matrix.FINDER_SIZE - Matrix.VERSION_SIZE + row,
        v,
        id
      );
      this.setModule(
  ...
  applyMaskPattern(maskPattern: any) {
  ...
  evaluatePenalty() {
    ...
    // Feature 4: Dark module percentage: 50 +|- (5*k) to 50 +|- (5*(k+1)), penalty points: N4 * k
    {
      ...
      // Deviation from 50%
      let percentage =
        (100 * darkCount + (this.dimension * this.dimension) / 2) /
...
class QrCode {
  ...
  static dataCapacity(version: any, errorCorrectionLevel: any) {
    const capacityCodewords = Math.floor(QrCode.totalDataModules(version) / 8);
    const eccTotalCodewords =
      QrCode.eccBlockCodewords(version, errorCorrectionLevel) *
      QrCode.eccBlockCount(version, errorCorrectionLevel);
  ...
  static eccBlockCount(version: number, errorCorrectionLevel: number) {
    const eccBlockCountLookup = [
      [
        0, 1, 1, 1, 2, 2, 4, 4, 4, 5, 5, 5, 8, 9, 9, 10, 10, 11, 13, 14, 16, 17,
  ...
  static eccBlockCodewords(version: number, errorCorrectionLevel: number) {
    const eccBlockCodewordsLookup = [
      [
        0, 10, 16, 26, 18, 24, 16, 18, 22, 22, 26, 30, 22, 22, 24, 24, 28, 28,
        26, 26, 26, 26, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28,
        28, 28, 28, 28, 28,
      ], // 0b00 Medium
      [
        0, 7, 10, 15, 20, 26, 18, 20, 24, 30, 18, 20, 24, 26, 30, 22, 24, 28,
        30, 28, 28, 28, 28, 30, 30, 26, 28, 30, 30, 30, 30, 30, 30, 30, 30, 30,
        30, 30, 30, 30, 30,
      ], // 0b01 Low
      [
        0, 17, 28, 22, 16, 22, 28, 26, 26, 24, 28, 24, 28, 22, 24, 24, 30, 28,
        28, 26, 28, 30, 24, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30,
        30, 30, 30, 30, 30,
      ], // 0b10 High
      [
        0, 13, 22, 18, 26, 18, 24, 18, 22, 20, 24, 28, 26, 24, 20, 30, 24, 28,
  ...
  static measureSegments(segments: any, version: any) {
    let total = 0;
  ...
  static tryToImproveErrorCorrectionLevel(
    segments: Segment[],
    version: number,
    currentErrorCorrectionLevel: number
  ) {
  ...
  static writePadding(
    ...
    remaining = Math.min(
      dataCapacity - scratchBuffer.position(),
      Segment.MODE_BITS
    );
    ...
    console.assert(
      scratchBuffer.position() === dataCapacity,
      "Unexpectedly failed to correctly fill the data buffer"
    );
  }
  ...
  static calculateEccAndFillMatrix(
    ...
    // Number of error correction codewords in each block
    const eccCodewords = QrCode.eccBlockCodewords(
      version,
    ...
    const eccOffset = Math.floor(
      (totalCapacity - 8 * eccCodewords * eccBlockCount) / 8
    );

    console.assert(
      8 * eccOffset === scratchBuffer.bitOffset,
      `Expected current bit position ${
        scratchBuffer.bitOffset
      } to match ECC offset *8 ${8 * eccOffset}`
    );
    ...
    const countShortBlocks =
      eccBlockCount - (dataCapacityBytes - dataLenShort * eccBlockCount);
    const dataLenLong =
      dataLenShort + (countShortBlocks >= eccBlockCount ? 0 : 1);
    ...
    for (let i = 0; i < dataLenLong; i++) {
      for (let block = 0; block < eccBlockCount; block++) {
        ...
        const codeword =
          block * dataLenShort +
          (block > countShortBlocks ? block - countShortBlocks : 0) +
          i;
  ...
  static generate(text: any, userOptions: any) {
    // Generation options
    ...
    const version = QrCode.findMinimumVersion(
      segments,
      errorCorrectionLevel,
      options.minVersion,
      options.maxVersion
    ...
    if (options.optimizeEcc) {
      errorCorrectionLevel = QrCode.tryToImproveErrorCorrectionLevel(
        segments,
        version,
        errorCorrectionLevel
      );
    ...
    QrCode.calculateEccAndFillMatrix(
      scratchBuffer,
      version,
      errorCorrectionLevel,
      matrix
    ...
    const formatInfo = QrCode.calculateFormatInfo(
      errorCorrectionLevel,
      maskPattern
    );
  ...
  static render(mode: string, matrix: any, renderOptions: any) {
    const renderers: any = {
      debug: renderDebug,
...
function BitmapGenerate(
  ...
  const stride =
    4 * Math.floor((width * Math.floor((bitsPerPixel + 7) / 8) + 3) / 4); // Byte width of each line
...
function renderDebug(
  matrix: {
    quiet: number;
    dimension: any;
    identifyModule: (arg0: number, arg1: number) => any;
    getModule: (arg0: number, arg1: number) => any;
    invert: any;
  },
  options: { segments: { [x: string]: any }; sep: string | undefined }
) {
...
function renderTextLarge(
  ...
  options: { segments: string | any[]; sep: string | undefined }
) {
  options = Object.assign(
    {
      segments: ["  ", "██"],
...
function renderTextMedium(
  ...
  options: { segments: any[]; sep: string | undefined }
) {
  options = Object.assign(
    {
      segments: [" ", "▀", "▄", "█"],
...
function renderTextCompact(
  ...
  for (let y = -matrix.quiet; y < matrix.dimension + matrix.quiet; y += 2) {
    ...
    for (let x = -matrix.quiet; x < matrix.dimension + matrix.quiet; x += 2) {
      let value = 0;
      value |= (matrix.getModule(x, y) ? !matrix.invert : matrix.invert)
        ? 0x01
        : 0x00;
      value |= (
        (x + 1 < matrix.dimension ? matrix.getModule(x + 1, y) : 0)
          ? !matrix.invert
          : matrix.invert
      )
        ? 0x02
        : 0x00;
      value |= (
        (y + 1 < matrix.dimension ? matrix.getModule(x, y + 1) : 0)
          ? !matrix.invert
          : matrix.invert
      )
        ? 0x04
      ...
      value |= (
        (
          y + 1 < matrix.dimension && x + 1 < matrix.dimension
            ? matrix.getModule(x + 1, y + 1)
            : 0
        )
          ? !matrix.invert
          : matrix.invert
...
function renderSvg(
  ...
  options: {
    ...
    moduleRound: any;
    white: any;
    finderRound: number | null;
    alignmentRound: number | null;
  }
) {
  options = Object.assign(
    {
  ...
  const vbWidthHeight = `${
    2 * (matrix.quiet + options.moduleSize / 2) + matrix.dimension - 1
  }`;
  ...
  lines.push(
    `<rect id="b" x="${-options.moduleSize / 2}" y="${
      -options.moduleSize / 2
    }" width="${options.moduleSize}" height="${options.moduleSize}" rx="${
      0.5 * (options.moduleRound || 0) * options.moduleSize
    }" />`
  );
...
function renderSvgUri(matrix: any, options: any) {
...
function renderBmpUri(matrix: any, options: any) {
  const bmpData = renderBmp(matrix, options);
  const encoded = btoa(
    new Uint8Array(bmpData).reduce(
      (data, v) => data + String.fromCharCode(v),
      ""
    )
  );
